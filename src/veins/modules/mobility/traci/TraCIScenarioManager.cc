...
void TraCIScenarioManager::deleteManagedModule(std::string nodeId)
{
    ...
    if (cc && nic) {
        cc = ChannelAccess::getConnectionManager(nic);
        cc->unregisterNic(nic);
    }
    cModule* nicVlcHead = mod->getSubmodule("nicVlcHead");
    if (nicVlcHead) {
        cc = ChannelAccess::getConnectionManager(nicVlcHead);
        cc->unregisterNic(nicVlcHead);
    }

    cModule* nicVlcTail = mod->getSubmodule("nicVlcTail");
    if (nicVlcTail) {
        cc = ChannelAccess::getConnectionManager(nicVlcTail);
        cc->unregisterNic(nicVlcTail);
    }
